# 自律型Agent + RAG (Ollama ローカルLLM版) プロジェクト
#### (1) 自律型Agent（Ollama ローカルLLM利用、スクラッチで作成）
...
- 計画生成は二層方式を採用しており、単純なクエリはルールベースで即時に計画を作り（LLM呼び出しなし）、複雑なクエリや明示的なWeb検索指示のあるクエリのみ ローカルLLM（Ollama）で計画を生成します。
- LLM 呼び出しは `grace/llm_compat.py` の `create_chat_client()` で生成したクライアント経由で行います。
- このクライアントは google-genai 互換の `client.models.generate_content(...)` インターフェースを保ったまま、内部では `create_llm_client("ollama")` 経由で Ollama（既定 `gemma4:e4b`、代替 `llama3.2`）を呼び出すアダプターです。Embedding（検索）は別途 Ollama Embedding `nomic-embed-text`（768次元）を使用します。

### Planner主な責務
- ユーザークエリの複雑度推定（キーワードベース / LLMベース）
- 二層方式による実行計画の生成（ルールベース計画 / LLM計画の振り分け）
- ローカルLLM（Ollama）を用いた実行計画の自動生成
...
- LLM呼び出しは`grace/llm_compat.py`の互換クライアント（`create_chat_client`）経由で Ollama（デフォルト `gemma4:e4b`）に委譲され、
- Embedding は Ollama Embedding（`nomic-embed-text`、768次元）を継続利用します。
...
- 本プロジェクトでは Ollama（既定 `gemma4:e4b`）が実体となります。
- 一方、ソース一致度計算の Embedding は Ollama Embedding（`nomic-embed-text`、768次元）を継続利用します。
...
- ローカルLLM（Ollama `gemma4:e4b`）やEmbedding（Ollama Embedding `nomic-embed-text`）のAPIを直接呼び出しません。
...
- ローカルLLM（Ollama、既定 `gemma4:e4b`）の呼び出しは `planner.py` を経由します。
...
> | 8 | [Streamlit アプリ設計書 (docs/agent_rag.md)](./docs/agent_rag.md) | agent_rag.py のアーキテクチャ・ページ構成・Ollama 設定・モデル一覧 |
...
> | # | ドキュメント | 説明 |
> | - | ----------- | ---- |
> | R1 | [LLM API 4プロバイダー対比表 v3 (docs/llm_api_comparison_v3.md)](./docs/llm_api_comparison_v3.md) | Ollama / Anthropic / OpenAI / Gemini の API・クライアント・Embedding 完全比較（最新版） |
>
> **アーカイブ**
>
...
- #9（ReAct+Reflection）と #10（GRACE）は別系統のエージェントです。ReAct は `services/agent_service.py`（`create_llm_client("ollama")` + Tool Use `generate_with_tools`／`stop_reason=="tool_use"` の ReAct ループ＋Reflection フェーズ）、GRACE は `grace/` パッケージ（Plan→Execute→Confidence→Intervention→Replan、`grace/llm_compat.py` 経由で Ollama 既定）。いずれも LLM は Ollama（既定 `gemma4:e4b`）。
- 技術スタック規約：LLM = Ollama（`gemma4:e4b`、`grace/llm_compat.py` 経由）、Embedding = Ollama Embedding（`nomic-embed-text`・768次元）。チャンキング/Q&A 生成の CLI ツール（`chunking/`・`qa_qdrant/`）は argparse 既定が `gemma4:e4b` で、`--model llama3.2` 等で上書き可能。APIキーは不要（ローカル実行、任意 `OLLAMA_BASE_URL`、既定 `http://localhost:11434/v1`）。
...
| 役割                              | プロバイダー  | モデル / 設定                                                   |
| --------------------------------- | ------------- | -------------------------------------------------------------- |
| LLM（計画策定・実行・応答生成）   | **Ollama**    | デフォルト `gemma4:e4b`（代替 `llama3.2`）、APIキー不要（ローカル実行、任意 `OLLAMA_BASE_URL` 既定 `http://localhost:11434/v1`）、ルーティングは `grace/llm_compat.py` 経由 |
| Embedding（Qdrant登録・検索）     | **Ollama**    | `nomic-embed-text`（768次元）、APIキー不要（ローカル実行）      |
| ベクトルDB                        | Qdrant        | `http://localhost:6333`                                        |

LLM プロバイダー / モデルは `grace/config.py` の `LLMConfig`（`provider="ollama"`, `model="gemma4:e4b"`）、Embedding は `EmbeddingConfig`（`provider="ollama"`, `model="nomic-embed-text"`, `dimensions=768`）が既定値です。ローカル実行のため API コストは発生しません（トークン集計のみ）。

**Version 4.3** | 最終更新: 2026-06-21
...
| モデル名                        | 説明                                       |
| ------------------------------- | ------------------------------------------ |
| `gemma4:e4b`                    | Gemma（**デフォルト**）                    |
| `llama3.2`                      | 軽量・高速モデル（代替）                    |
...
**概要**: ユーザーの質問を分析し、ローカルLLM（Ollama）を使って ExecutionPlan を生成する。
...
| ----------- | ------------------------------------------------------------------------------------------------------- |
| **Input**   | `query: str` — ユーザーの質問文                                                                        |
| **Process** | `COMPLEXITY_ESTIMATION_PROMPT` を使い、Ollama API で数値のみを取得（temperature=0.1）                  |
...
| `openai`        | >= 1.0     | Ollama OpenAI互換クライアント（`/v1`、LLM + Embedding 共通）|
...
| `config.ModelConfig`        | Ollama モデル設定（利用可能モデル一覧）                        |
...
flowchart TB
    ...
    subgraph External["外部サービス"]
        Qdrant["Qdrant"]
        ClaudeAPI["Ollama LLM API"]
        GeminiEmbed["Ollama Embedding API"]
...
| `ReasoningTool` | `reasoning`  | LLM推論（検索結果を基にOllamaで回答生成）                          |
...
| `LLMSelfEvaluator`        | Ollama による自己評価（query と answer の整合性）          |
...
| **4.3**    | **2026-06-21** | Ollama ネイティブ化の表記統一・Mermaid §7 スタイル整備。主な変更: (1) LLM = Ollama（既定 `gemma4:e4b`・代替 `llama3.2`、`grace/llm_compat.py` 経由 `create_llm_client("ollama")`）、Embedding = Ollama Embedding（`nomic-embed-text`・768次元）に表記統一、(2) APIキー不要（ローカル実行・任意 `OLLAMA_BASE_URL` 既定 `http://localhost:11434/v1`）に更新し、per-token USD コスト記述を「ローカル実行のため API コストは発生しない（トークン集計のみ）」へ、(3) 設定リファレンス（`ModelConfig` / `LLMConfig` / `EmbeddingConfig` / `CostConfig`）・依存ライブラリ・トラブルシューティングを Ollama 前提に修正、(4) Mermaid 図のラベルを Ollama 表記に更新（ノードIDは保持）し §7 黒背景スタイルを再確認 |
...
| 設定項目              | デフォルト値             | 説明                          |
| --------------------- | ------------------------ | ----------------------------- |
| `DEFAULT_MODEL`       | `"gemma4:e4b"`           | デフォルト LLM モデル         |
| `LIGHT_MODEL`         | `"llama3.2"`             | 軽量 LLM モデル（代替）       |
| `EMBEDDING_MODEL`     | `"nomic-embed-text"`     | 埋め込みモデル（Ollama）      |
...
| サブ設定        | 主要項目                                           | 説明           |
| --------------- | -------------------------------------------------- | -------------- |
| `CostConfig`    | （ローカル実行のため API コストは発生しない — トークン集計のみ） | コスト管理     |
...
| Planner/Executor 初期化エラー            | Ollama サーバー未起動 / モデル未取得 | `ollama serve` を起動し `ollama pull gemma4:e4b` / `ollama pull nomic-embed-text` を実行（APIキーは不要）。任意で `OLLAMA_BASE_URL`（既定 `http://localhost:11434/v1`）を確認 |
| GraceConfig 読み込みエラー               | YAML ファイル不在 or 構文エラー | `config/grace_config.yml` を確認。不在時はデフォルト値で動作         |
| 計画策定が常にフォールバック             | LLM呼び出し失敗                 | Ollama サーバー起動・モデル取得・モデル名を確認。ログ`logs/grace_run.log` 参照 |
