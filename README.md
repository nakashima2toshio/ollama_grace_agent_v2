...
> | # | ドキュメント | 説明 |
> | - | ----------- | ---- |
> | R1 | [LLM API 4プロバイダー対比表 v3 (docs/llm_api_comparison_v3.md)](./docs/llm_api_comparison_v3.md) | Ollama / OpenAI / Anthropic / Gemini の API・クライアント・Embedding 完全比較（参考資料） |
...
flowchart TB
    ...
    subgraph Planner_Layer["Planner Layer (grace/planner.py)"]
        ...
        PlanGeneration["計画生成<br/>(Ollama JSON モード)"]
    end
...
| モデル名                     | 説明                                          |
| ---------------------------- | --------------------------------------------- |
| `gemma4:e4b`                 | Google Gemma 4 4B / ローカル（**デフォルト推奨**） |
...
sequenceDiagram
    ...
    Planner->>Planner: 5. LLM計画生成 (Ollama JSON モード)
...
**概要**: ユーザーの質問を分析し、LLM（Ollama）を使って ExecutionPlan を生成する。
...
| **Process** | 1.`_get_available_collections()` — Qdrantから利用可能コレクション一覧を取得<br>2. `estimate_complexity_with_llm(query)` — LLMで複雑度を推定（0.0-1.0）<br>3. `PLAN_GENERATION_PROMPT` にコレクション一覧・クエリを埋め込み<br>4. Ollama（OpenAI互換API）で JSON モード出力（`response_format=ExecutionPlan`）<br>5. `ExecutionPlan.model_validate_json()` でパース<br>6. `validate_plan_dependencies()` で依存関係を検証<br>7. 計画IDを付与（`create_plan_id()`） |
...
| ----------- | ------------------------------------------------------------------------------------------------------- |
| **Input**   | `query: str` — ユーザーの質問文                                                                        |
| **Process** | `COMPLEXITY_ESTIMATION_PROMPT` を使い、Ollama（OpenAI互換API）で数値のみを取得（temperature=0.1）       |
...
| `openai`        | >= 1.100.2 | OpenAI互換クライアント（Ollamaエンドポイント）|
...
| `config.GeminiConfig`       | Ollamaモデル設定（ローカルLLM・利用可能モデル一覧）            |
...
### B.2 GeminiConfig（Ollamaモデル設定 — ローカルLLM）


| 設定項目              | デフォルト値          | 説明                                       |
| --------------------- | --------------------- | ------------------------------------------ |
| `DEFAULT_MODEL`       | `"gemma4:e4b"`        | デフォルトモデル（`OLLAMA_DEFAULT_MODEL` で上書き可） |
| `EMBEDDING_MODEL`     | `"nomic-embed-text"`  | 埋め込みモデル（Ollama Embedding）         |
...
| サブ設定        | 主要項目                                           | 説明           |
| --------------- | -------------------------------------------------- | -------------- |
| `OllamaConfig`  | `base_url="http://localhost:11434/v1"`             | Ollamaエンドポイント（`OLLAMA_BASE_URL` で上書き可・APIキー不要） |
...
| Planner/Executor 初期化エラー            | Ollama サーバー未起動           | Ollama サーバが起動しているか確認（APIキー不要・`OLLAMA_BASE_URL`） |
