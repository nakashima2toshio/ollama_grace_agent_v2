...
- [3. 環境変数設定](#3-環境変数設定)
  ...
  - [3.2 Ollamaのインストール・モデル取得](#32-ollamaのインストールモデル取得)
...
graph TD
    ...
    Streamlit -->|Q&A生成/Embedding| Ollama(Ollama<br>ローカルLLM<br>Port: 11434)
...
style BackgroundJobs fill:#1a1a1a,stroke:#fff,color:#fff
```
...
```bash
# プロジェクトディレクトリに移動
cd /path/to/ollama_grace_agent_v2
...
(venv) $ which python
/path/to/ollama_grace_agent_v2/venv/bin/python
```
...
# requirements.txtからインストール (Celery, Ollama(OpenAI互換クライアント), Qdrant等)
...
| パッケージ          | バージョン | 用途                                 |
| ------------------- | ---------- | ------------------------------------ |
| openai              | 1.100.2    | Ollama OpenAI互換クライアント(キー不要) |
...
本システムはローカル実行のためAPIキー不要で、`.env` は必須ではありません。
接続先(Ollama/Qdrant/Redis)をデフォルトから変更する場合のみ作成します。
...
### 3.2 Ollamaのインストール・モデル取得

本システムはローカルLLM(Ollama)を使用するため、**APIキーは不要**です。
有料APIキーの取得手続きは一切ありません。Ollama本体をインストールし、モデルをローカルにpullするだけで動作します。
...
# Windows: https://ollama.com/download からインストーラを取得
```

#### Ollamaサーバーの起動

```bash
# Ollamaサーバーを起動(デフォルト: http://localhost:11434)
ollama serve
```
...
```bash
# LLM(デフォルト: gemma4:e4b / 代替: llama3.2)
...
.env ファイルは必須ではありません(ローカル実行のためAPIキー不要)。
接続先をカスタマイズする場合のみ、以下を設定可能です。

```bash
# === オプション ===
# Ollama Base URL(デフォルト: http://localhost:11434/v1)
...
```bash
# Ollamaサーバーへの接続とモデルの存在を確認
curl -s http://localhost:11434/api/tags | grep -E "gemma4|nomic-embed-text" && echo " OK" || echo " NG"
```
...
Q&A生成処理では、Ollama(ローカルLLM)を大量に呼び出します。同期処理では:
...
| Ollama    | 11434  | curl localhost:11434/api/tags | モデル一覧JSON     |
...
**エラー:** Connection refused: localhost:11434
...
**エラー:** model "gemma4:e4b" not found, try pulling it first

**対処:**

* **原因**: 指定モデルがローカルに存在しません。
* **対策**:
  1. `ollama pull gemma4:e4b`(または `ollama pull llama3.2`)でモデルを取得する
  2. Embedding用に `ollama pull nomic-embed-text` を取得する
  3. メモリ不足で大きいモデルが動かない場合は、Celeryのワーカー数を減らす (`./start_celery.sh restart -w 4`)

> 補足: Ollamaはローカル実行のため、APIキーやレート制限・利用コストはありません。
...
| Ollama           | 11434  | ローカルLLM/Embedding |
...
> APIキーは不要です(ローカル実行)。`.env` は接続先カスタマイズ時のみ作成してください。
