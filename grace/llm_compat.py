...
[MIGRATION anthropic→ollama]
    - DEFAULT_ANTHROPIC_MODEL / claude-sonnet-4-6 → DEFAULT_OLLAMA_MODEL / gemma4:e4b
    - AnthropicGenaiClient → OllamaGenaiClient（helper.helper_llm.OllamaClient をラップ）
    - API キー不要（ローカル実行）
    - 既定 provider="ollama"（gemini 指定時のみ google-genai を使用）
