...
システムの特徴（ReAct + Reflection、フルスクラッチ実装、Ollama Tool Use ネイティブ）です。
...
> **Ollama（ローカル LLM、既定 `gemma4:e4b` / 代替 `llama3.2`）** を **Ollama の OpenAI 互換 Tool Use** で
> 駆動します（旧 Gemini ネイティブ function-calling 実装から移行済み）。具体的には:
>
> - `helper/helper_llm.py` の `create_llm_client("ollama")` を経由して `OllamaClient` を生成し、
>   `self.llm.generate_with_tools(messages, tools, system, max_tokens)` で推論します。
> - レスポンスはタプル `(text, tool_calls, finish_reason)` として返り、
>   ループ継続は **`finish_reason == "tool_calls"`** で判定します。
> - 会話履歴は OpenAI 互換のステートレス設計に合わせ **`self._messages`（dict のリスト）で自前管理**し、
>   `execute_turn()` の先頭でリセットします。
> - 既定モデルは `get_config("models.default", "gemma4:e4b")` で解決します
>   （旧実装にあった「非 gemini モデル名の自動フォールバック」ロジックは廃止済み）。
> - **Embedding は Ollama `nomic-embed-text`（768次元）** を使用します（検索側の責務）。
> - **API キーは不要（ローカル実行）。** 接続先は任意で `OLLAMA_BASE_URL`（既定 `http://localhost:11434/v1`）で指定します。
>   ローカル実行のため API コストは発生しません（トークン集計のみ）。
...
3. フルスクラッチ実装:
   　　Ollama の OpenAI 互換 Tool Use を helper.helper_llm 経由で利用し、柔軟な制御を実現しました。
   　　会話状態は self._messages リストで自前管理します（OpenAI 互換のステートレス設計）。
4. スマート検索（キャッシュ + 並列検索）:
...
- **Embedding** は **Ollama** (`nomic-embed-text` / 768次元) を採用

> **LLM の扱い（重要）:** `ReActAgent` は **Ollama（ローカル LLM、既定 `gemma4:e4b` / 代替 `llama3.2`）** を
> **Ollama の OpenAI 互換 Tool Use** で駆動します（`create_llm_client("ollama")` 経由。
> 既定 `gemma4:e4b`。旧 Gemini 実装から移行済み）。
...
| ファイル名     | agent_rag.py                                                                  |
| フレームワーク | Streamlit                                                                     |
| プロジェクト標準LLM | Ollama（ローカル LLM、`gemma4:e4b` / 代替 `llama3.2`）                    |
| ReActエージェント実行LLM | **Ollama の OpenAI 互換 Tool Use**（既定 `gemma4:e4b`）             |
...
| エージェント対話 | 🤖       | **ReAct + Reflection Agent** (Ollama Tool Use) との対話。ナレッジベース検索 + **Reflection (自己推敲)** による高品質な回答。 |
| 未回答ログ       | 📊       | エージェントが回答できなかった質問のログ分析                                                               |
| RAGデータDL      | 📥       | HuggingFace/ローカルファイルからデータ取得・前処理                                                         |
| Q/A生成          | 🤖       | LLM によるQ&Aペア自動生成（Celery並列処理対応）                                                          |
| CSVデータ登録    | 📥       | **Ollama Embedding (768次元)** でベクトル化・登録・コレクション統合                                       |
...
graph TD
    ...
    subgraph DataAccess ["データアクセス層"]
        LLM["Ollama (ReAct LLM: gemma4:e4b 既定 / helper.helper_llm 経由)"]
        EmbedAPI["Ollama Embedding (Embed: nomic-embed-text / 768次元)"]
        DB["Qdrant"]
    end
...
graph LR
    ...
    subgraph Helper_Layer ["Helpers"]
        LLMHelper["helper/helper_llm.py (create_llm_client / OllamaClient)"]
        HelperEmbed["helper/helper_embedding.py (OllamaEmbedding)"]
...
| **LLMクライアント**  | `helper/helper_llm.py`（`OllamaClient`） | `create_llm_client("ollama")` / `generate_with_tools()`（Ollama の OpenAI 互換 Tool Use を抽象化） |
| **Embedding抽象化**  | `helper/helper_embedding.py`        | `OllamaEmbedding` / `create_embedding_client("ollama")`           |
...
graph TB
    ...
    subgraph External ["External"]
        Claude["Ollama (gemma4:e4b / helper.helper_llm 経由)"]
        Gemini["Ollama Embedding (nomic-embed-text)"]
    ...
    Claude -- "(text, tool_calls, finish_reason==tool_calls)" --> ReAct
    ReAct -- "Execute" --> Tools
...
1. データDL -> 2. 前処理（プロンプト最適化、チャンク） -> 3. QA生成 (LLM) -> 4. 埋め込み登録 (Ollama Embedding) -> **5. エージェントによる活用 (Search & Answer)**
...
*   **クラス `ReActAgent`**:
    *   **LLMクライアント**: `create_llm_client("ollama", default_model=self.model_name)` で `OllamaClient` (`self.llm`) を生成（API キーは不要。接続先は任意で `OLLAMA_BASE_URL`、既定 `http://localhost:11434/v1`）。クライアント／チャットセッションの初期化は廃止し、`_build_system_instruction()` / `_build_tools()` に置き換え済み（旧 Gemini 実装から移行済み）。
    *   **system / tools**: `_build_system_instruction()` が `system=` パラメータで渡す system プロンプトを構築し、`_build_tools()` が `input_schema` 形式の dict リスト（`search_rag_knowledge_base` / `list_rag_collections`）を返します。会話履歴は `self._messages`（dict のリスト）で自前管理します（OpenAI 互換のステートレス設計）。
    *   **モデル解決**: `model_name or get_config("models.default", "gemma4:e4b")`。旧実装にあった「非 gemini モデル名の自動フォールバック」ロジックは廃止済みです。
    *   **ReActループ** (`_execute_react_loop`): `generate_with_tools()` を駆動し、`finish_reason == "tool_calls"` を検出してツール実行を制御。
...
> `_build_tools()` が **Tool Use 形式（`input_schema` の dict リスト）** として定義し、
...
* **機能**: Ollama の OpenAI 互換 Tool Use を用いた `ReActAgent` とのチャットインターフェース。
* **特徴**:
  * **モデル選択**: サイドバーで Ollama 系モデル（`gemma4:e4b` / `llama3.2` / `gemma2`）から使用モデルを選択可能。既定は `AgentConfig.MODEL_NAME`（= `GeminiConfig.DEFAULT_MODEL` = `gemma4:e4b`）。
...
Ollama の OpenAI 互換 Tool Use を利用し、以下のサイクルを回します。
![ReAct_fig](doc/assets/agent_5_react.png)

1. **ReAct フェーズ (解決) — `_execute_react_loop`**:

   * **Thought (思考)**: ユーザーの入力に対し、外部知識が必要か、どんなクエリで検索すべきか考える。
   * **Action (行動)**: ツール (`search_rag_knowledge_base`) を呼び出す（`generate_with_tools()` の戻り値タプル `(text, tool_calls, finish_reason)` の `finish_reason == "tool_calls"` と `tool_calls` の有無で判定）。
   * **Observation (観察)**: ツールを実行し、その結果を `{"type":"tool_result", "tool_use_id":..., "content":...}` として（複数ツールは1件の user メッセージにまとめて）`self._messages` に追記し、次ループで Ollama に再送。
   * **Draft Answer (ドラフト作成)**: `finish_reason` が `"tool_calls"` でなくなったターンのテキスト（`text`）を回答案 (`final_text`) とする。
...
| **Process** | 1. `self.thought_log` と `self._messages` をリセット（OpenAI 互換のステートレス設計）。<br>2. **ReAct Loop** (`_execute_react_loop`):<br>　a. （任意）`KeywordExtractor` でキーワード抽出し入力を拡張、`self._messages` に user メッセージとして追記。<br>　b. `self.llm.generate_with_tools(messages, tools, system, max_tokens)` を呼ぶ。<br>　c. 戻り値タプル `(text, tool_calls, finish_reason)` の `finish_reason == "tool_calls"` かつ `tool_calls` ありならツール実行（`search_rag_knowledge_base` → `search_rag_knowledge_base_cached`）。<br>　d. `assistant_message` を履歴へ追記し、全 `tool_result`（`tool_use_id` 一致）を1件の user メッセージにまとめて追記して `max_turns`(既定10) まで反復。<br>　e. `finish_reason` が `"tool_calls"` でなければそのテキストを `final_text` イベントとしてYield。<br>3. **Reflection Phase** (`_execute_reflection_phase`):<br>　a. `REFLECTION_INSTRUCTION` + 回答案を `self._messages` に追記し `generate_with_tools(tools=[])` で送信。<br>　b. 戻り値タプルの `text` を抽出し、`Final Answer:` で分割。<br>　c. `Final Answer:` 以降を最終回答とする。<br>4. `_format_final_answer` で整形し `final_answer` イベントをYield。 |
...
| `tool_call`     | `name`, `args`       | Tool Use 検出時（`finish_reason == "tool_calls"`） | 思考プロセスに追記し `st.spinner` で実行中表示         |
...
本ドキュメントは、ReAct + Reflection エージェントの理論的背景（概念図）と、`agent_rag.py` および関連モジュールの実装詳細を体系的にまとめたリファレンスです。本エージェントの推論ロールは Ollama の OpenAI 互換 Tool Use（`helper.helper_llm` 経由、既定 `gemma4:e4b`）で実行されます。

---

## 第2部: アーキテクチャ概念 (Theoretical Architecture)

エージェント (Ollama / ローカル LLM) の思考プロセスは、大きく2つのフェーズ（解決と推敲）で構成されています。
...
最終的な検索クエリの決定は **LLM (Ollama / ローカル LLM) モデル自体** が、システムプロンプトの指示に基づき「入力文」を解釈し「最適な検索クエリ」へと変換（推論）します。
...
* コード処理 (`ReActAgent.execute_turn` → `_execute_react_loop`):
  ...
  `self.llm.generate_with_tools(messages=self._messages, ...)` 経由で Ollama に送信されます。
...
2. 生成・推論フェーズ (Ollama / ローカル LLM 内部)
...
* コード処理 (`_execute_react_loop` → `search_rag_knowledge_base_cached`):
  Ollama から返ってきた `tool_calls` の各要素（モデルが決めたクエリ。`tc["name"]` / `tc["input"]` / `tc["id"]`）を Python 側で受け取り、検索関数を実行します。
...
| 2. 変換  | Ollama / ローカル LLM | 文脈から「検索用クエリ」を推論・生成する | 「実験生物学 実験操作」 (ケースB)       |
| 3. 実行  | agent_tools.py     | 生成されたクエリで全コレクション検索     | query="実験生物学 実験操作" で検索      |

つまり、「クエリ生成ロジック」の実体は主に LLM（Ollama / ローカル LLM）の推論プロセスの中にあります（`KeywordExtractor` は補助）。
...
| 2. 自己評価 | `Ollama / ローカル LLM`            | 基準（正確性・スタイル）に従って評価 | 「自己評価: ...修正は不要と判断しました。」            |
| 3. 最終化   | `Ollama / ローカル LLM`            | 修正版（またはそのまま）の回答を出力 | 「社内ナレッジによると...（最終回答）」                |
...
* **`ReActAgent.__init__(selected_collections, model_name=None, session_id=None, use_hybrid_search=True)`**:
  * **役割**: エージェントを初期化します。`create_llm_client("ollama", default_model=self.model_name)` で `OllamaClient` (`self.llm`) を生成し、`_build_system_instruction()` が system プロンプトを、`_build_tools()` が `input_schema` 形式の Tool Use 定義を事前構築します。会話履歴は `self._messages = []` で初期化します。
  * **詳細**: `model_name` 未指定時は `get_config("models.default", "gemma4:e4b")` を使用。旧実装にあった「非 gemini モデル名の自動フォールバック」ロジックは廃止済み。UI で選択した Ollama モデルを動的に切替可能。
...
Ollama の OpenAI 互換 Tool Use と Python の `for turn_count in range(1, max_turns + 1):` ループを組み合わせて ReAct を実現しています。
...
flowchart TD
    ...
    subgraph Agent_Process ["execute_turn メソッド"]
        ...
        subgraph ReAct_Phase ["Phase 1: _execute_react_loop"]
            Think["思考 Thought"] --> Decide{"finish_reason == tool_calls ?"}
...
* **ツール検出**: `generate_with_tools()` の戻り値タプル `(text, tool_calls, finish_reason)` の `finish_reason == "tool_calls"` かつ `tool_calls` ありで判定。`finish_reason` が `"tool_calls"` でないターンで `break` してドラフトを確定します。
* **Thoughtの可視化**: テキスト (`text`) に `Thought:` または `考え:` を含む場合、`log` イベントとしてYieldし Streamlit UI (`st.expander`) にリアルタイム表示します。
...
> **ツール定義 (`_build_tools`)**: `_build_tools()` が **Tool Use 形式（`{"name", "description", "input_schema"}` の dict リスト）** を返し、
...
`helper/helper_embedding.py` に集約・抽象化されています。Qdrant ラッパー (`qdrant_client_wrapper.embed_query`) は内部で Ollama Embedding に委譲します。


| 項目               | 詳細                                                                                                              |
| :----------------- | :---------------------------------------------------------------------------------------------------------------- |
| **抽象基底クラス** | `EmbeddingClient`（`helper/helper_embedding.py`）                                                                  |
| **実装クラス**     | **`OllamaEmbedding`**: Ollama Embeddings API (`nomic-embed-text` / 768次元) を使用。現行の主力。                   |
...
graph TD
    subgraph PythonApp ["Python App"]
        ...
        Embed --> EmbedAPI["Ollama Embedding API"]
        EmbedAPI --> Embed
...
| **Embedding**     | `qdrant_client_wrapper.py`       | `embed_query()` / `embed_sparse_query_unified()` | クエリを Dense / Sparse ベクトルに変換します（Ollama Embedding）。            |
...
2. **ベクトル化 (Parallel)**:
   * **Dense**: `embed_query(query)` → `[0.12, -0.5, ...]` (768次元)
...
PointStruct(
    id=...,
    vector={
        "default": [0.1, 0.2, ...],        # Dense Vector (意味検索用 / 768次元)
...
sequenceDiagram
    ...
    participant LLM as "Ollama (helper.helper_llm)"
    ...
    loop 解決するまで繰り返し (max_turns)
        LLM-->>Svc: (text, tool_calls, finish_reason)
...
| **ツール検出**         | `finish_reason == "tool_calls" and tool_calls` | モデルがツール利用を要求した場合、対応する Python 関数 (`agent_tools.py`) を実行します。 |
...
| **LLMクライアント**    | `self.llm = create_llm_client("ollama", ...)` / `self.llm.generate_with_tools(...)` | `helper.helper_llm` 経由で Ollama の OpenAI 互換 Tool Use を利用。戻り値タプル `(text, tool_calls, finish_reason)` を解析します。 |
...
```bash
# ReAct エージェントの LLM / Embedding はいずれも Ollama（ローカル実行）のため API キーは不要です。
# 接続先を変更する場合のみ、任意で OLLAMA_BASE_URL を指定します。
OLLAMA_BASE_URL=http://localhost:11434/v1   # 任意（既定値）
...
> **注:** 本書の `ReActAgent` の LLM は **Ollama（ローカル LLM、既定 `gemma4:e4b`）** です。`create_llm_client("ollama")` は
> ローカルの OpenAI 互換エンドポイント（既定 `http://localhost:11434/v1`、api_key は `"ollama"` 固定）に接続するため、
> API キーは不要です。Embedding（検索）も Ollama `nomic-embed-text`（768次元）でローカル実行され、API コストは発生しません（トークン集計のみ）。
...
| 定数 / 設定                              | 場所                  | 値 / 既定                       | 用途                                   |
| :--------------------------------------- | :-------------------- | :------------------------------ | :------------------------------------- |
| `GeminiConfig.DEFAULT_MODEL`             | `config.py`           | `gemma4:e4b`                    | 既定の ReAct LLM モデル（Ollama 設定クラス）|
| UI モデル選択肢                          | `config.py`           | `gemma4:e4b` / `llama3.2` / `gemma2` | UI のモデル選択肢            |
| `AgentConfig.MODEL_NAME`                 | `config.py`           | `= GeminiConfig.DEFAULT_MODEL`  | エージェント既定モデル                 |
| `GeminiConfig.EMBEDDING_MODEL` / `EMBEDDING_DIMS` | `config.py`  | `nomic-embed-text` / `768`      | Embedding モデルと次元数               |
| `models.default`                         | `get_config(...)`     | `gemma4:e4b`                    | 既定 LLM モデル（未指定時に使用）      |
...
> **モデル名について:** Ollama モデル名はそのまま使用します（マッピングは作りません）。`ReActAgent` は
> `model_name or get_config("models.default", "gemma4:e4b")` で既定モデルを解決します
...
CLI 版エージェント（`agent_main.py` の `UpgradedCLIAgent.execute_turn`）も利用可能です。CLI 版も同じく Ollama の OpenAI 互換 Tool Use を利用し（`create_llm_client("ollama")` / `generate_with_tools`）、`AgentConfig.MODEL_NAME`（= `gemma4:e4b`）を既定モデルとします。ただし UI 版 `ReActAgent` と異なり、ストリーミングではなく最終回答文字列を返し、`search_rag_knowledge_base`（非キャッシュ版）を直接呼びます。

### 8.2 典型的なワークフロー

1. RAGデータをダウンロード・前処理（📥 RAGデータDL）。
2. Q/A を生成（🤖 Q/A生成）。
3. Ollama Embedding でベクトル化し Qdrant に登録（📥 CSVデータ登録）。
...
graph TD
    ...
    subgraph External_API ["External"]
        ClaudeLLM["Ollama (gemma4:e4b / OpenAI 互換)"]
        Gemini["Ollama Embedding (nomic-embed-text)"]
...
| バージョン | 日付 | 内容 |
|-----------|------|------|
| v4.0 | 2026-06-21 | **Ollama ネイティブ化の表記統一・Mermaid §7 スタイル整備。** LLM を `create_llm_client("ollama")`（`OllamaClient` / OpenAI 互換 Tool Use、既定 `gemma4:e4b` / 代替 `llama3.2`）に統一。Tool Use 判定を `finish_reason == "tool_calls"`、戻り値をタプル `(text, tool_calls, finish_reason)` に修正。Embedding を Ollama `nomic-embed-text`（768次元）へ統一。API キーは不要（ローカル実行・任意 `OLLAMA_BASE_URL`）、API コストなし（トークン集計のみ）に変更。設定参照を `GeminiConfig.DEFAULT_MODEL`(=`gemma4:e4b`)・`EMBEDDING_MODEL`(=`nomic-embed-text`/`768`)・`models.default`(=`gemma4:e4b`) に更新。全 Mermaid 図のラベルを Ollama フローへ更新（ノード ID・黒背景スタイルは維持）。 |
