...
- [x] **3-2.** 実際のコード設定と照合:
  - `config.py` → `QdrantConfig.DEFAULT_VECTOR_SIZE = 768`
  - `config.py` → `GeminiConfig.EMBEDDING_DIMS = 768`（クラス名は後方互換のため `GeminiConfig`・値は Ollama）
  - `qdrant_client_wrapper.py` → `PROVIDER_DEFAULTS["ollama"]["dims"] = 768`
- [x] **3-3.** ドキュメント内の次元数を正しい値（768 / `nomic-embed-text`）に修正 → **全12箇所修正完了**
- [x] **3-4.** 別プロバイダー・別次元の旧コレクションが混在しうる旨を注意書きとして追記 → **4.6 embed_query セクションに追記**
- [ ] **3-5.** 完了コミット
...
- [x] **9-1.** 現在の3つのエントリポイントの使用箇所を調査
  - `search_rag_knowledge_base`: TOOLS_MAP登録 + Ollama tools 定義用（OpenAI 互換 tool_calls）。実行時は agent_service が `search_rag_knowledge_base_cached` にインターセプト
...
- [x] **11-1.** 現在登録されている全コレクションの次元数を確認
  - `COLLECTION_EMBEDDINGS` (旧 1536次元): 定義のみ、検索ロジックで未参照
  - `COLLECTION_EMBEDDINGS_GEMINI` (旧 3072次元): 定義のみ、検索ロジックで未参照
  - 現在の検索は `DEFAULT_EMBEDDING_PROVIDER=ollama` (`nomic-embed-text` / 768次元) を使用
- [x] **11-2.** 別次元の旧コレクションが実際に使用されているか確認
...
agent_tools.py: search_rag_knowledge_base_cached()
  → parallel_search_engine.search_all_collections(query, all_collections, search_func_with_hybrid)
    → 各コレクションごとに search_rag_knowledge_base_structured(query, col) を並列実行
      → embed_query(query)            ← コレクションごとに毎回呼ばれる（Ollama Embedding）
...
- **呼び出し回数削減**: Ollama Embedding の呼び出し回数が N → 1 に（ローカル実行のため API コストは発生しない（トークン集計のみ）が、無駄な計算が削減される）
- **レイテンシ改善**: Embedding生成（約100-300ms）× N回 が1回に
- **負荷低減**: Ollama サーバへのリクエスト集中を回避
...
3. LLM(Ollama)が `Thought:` で思考し、`Action: search_rag_knowledge_base(query="...")` を生成
...
`config.py` で `QdrantConfig.DEFAULT_VECTOR_SIZE = 768`（`nomic-embed-text` / Ollama Embedding）であり、設計ドキュメント (`grace_chat_input_query.md`) も「768次元」で統一されている。

ただし、`COLLECTION_EMBEDDINGS` には別次元（旧プロバイダー）のコレクション定義が残存しており、それらに対して検索すると次元数不整合でエラーになるリスクがある。

現在 `embed_query()` は常に `provider="ollama"` (768次元) を使うため、別次元のコレクションに対して検索すると次元不一致エラーが発生する。
...
| 優先度 | # | 項目 | 効果 |
|--------|---|------|------|
| 🔴 高 | 1 | Embedding生成の重複呼び出し | 計算負荷・レイテンシ大幅削減（ローカル実行のため API コストは発生しない） |
