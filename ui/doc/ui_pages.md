...
#### 概要
Ollama（ローカルLLM・既定 `gemma4:e4b`／代替 `llama3.2`）を使用したReAct型エージェントとの対話インターフェース。Qdrant上のナレッジベースを動的に選択し、RAG検索を行いながら回答を生成。
...
| config | DATASET_CONFIGS, GeminiConfig | 設定値 |
...
#### 概要
Qdrantベクトルデータベースを使用した意味検索。検索結果に基づいてOllama（ローカルLLM）による応答を生成。
...
Dense（Ollama Embedding / `nomic-embed-text`・768次元）とSparse（FastEmbed）の両方のベクトルを生成・登録し、検索精度を向上。
