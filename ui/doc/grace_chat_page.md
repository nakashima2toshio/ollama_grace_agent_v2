...
`GeminiConfig` は Ollama のローカルLLMモデル設定を保持する設定クラスです（クラス名は維持）。

**オプション一覧** (`GeminiConfig.AVAILABLE_MODELS`):

| モデル名 | 説明 |
|---------|------|
| `gemma4:e4b` | Ollama ローカルLLM（**デフォルト**） |
| `llama3.2` | Ollama ローカルLLM（代替） |
...
**概要**: ユーザーの質問を分析し、LLM（Ollama / ローカルLLM）を使って ExecutionPlan を生成する。
...
| **Process** | 1. `_get_available_collections()` — Qdrantから利用可能コレクション一覧を取得<br>2. `estimate_complexity_with_llm(query)` — LLMで複雑度を推定（0.0-1.0）<br>3. `PLAN_GENERATION_PROMPT` にコレクション一覧・クエリを埋め込み<br>4. Ollama（OpenAI 互換 API）で JSON Schema 出力（`response_schema=ExecutionPlan`）<br>5. `ExecutionPlan.model_validate_json()` でパース<br>6. `validate_plan_dependencies()` で依存関係を検証<br>7. 計画IDを付与（`create_plan_id()`） |
...
|------|------|
| **Input** | `query: str` — ユーザーの質問文 |
| **Process** | `COMPLEXITY_ESTIMATION_PROMPT` を使い、Ollama（OpenAI 互換 API）で数値のみを取得（temperature=0.1） |
...
| `ollama` | >= 0.3 | Ollama ローカルLLM（OpenAI 互換 API クライアント） |
...
| `config.GeminiConfig` | Ollama モデル設定（利用可能モデル一覧。クラス名は維持） |
...
**デフォルトモデル**: `gemma4:e4b`（代替 `llama3.2`、`LLMConfig.model`）
...
| `LLMSelfEvaluator` | Ollama / ローカルLLM による自己評価（query と answer の整合性） |
...
1. ページにアクセス
2. サイドバーで必要に応じてモデルを変更（デフォルト: `gemma4:e4b`、代替 `llama3.2`）
...
| 2.1 | 2026-06-21 | Ollama ネイティブ化の表記統一・Mermaid §7 スタイル整備 |
...
| 設定項目 | デフォルト値 | 説明 |
|---------|-------------|------|
| `DEFAULT_MODEL` | `"gemma4:e4b"` | デフォルトモデル（代替 `llama3.2`） |
...
| 設定項目 | デフォルト値 | 説明 |
|---------|-------------|------|
| `provider` | `"ollama"` | LLMプロバイダー（`create_llm_client("ollama")`） |
| `model` | `"gemma4:e4b"` | 使用モデル（代替 `llama3.2`） |
...
| 設定項目 | デフォルト値 | 説明 |
|---------|-------------|------|
| `provider` | `"ollama"` | 埋め込みプロバイダー（`create_embedding_client("ollama")`） |
...
| サブ設定 | 主要項目 | 説明 |
|---------|---------|------|
| `CostConfig` | （ローカル実行のため API コストは発生しない。トークン集計のみ） | トークン集計 |
...
| Planner/Executor 初期化エラー | Ollama 未起動 | APIキーは不要（ローカル実行）。`ollama serve` を起動し、必要に応じて `OLLAMA_BASE_URL` を設定 |
