...
- URL: https://github.com/nakashima2toshio/ollama_grace_agent_v2
  [環境準備（超簡易版）]：
- 上記リポジトリーを自環境にcloneし、
...
- Ollama はローカル実行のため API キーは不要です。任意で `OLLAMA_BASE_URL`（既定 http://localhost:11434/v1）を環境変数に登録してください。

---

##### 開発環境（確認環境）：他の環境の方は、Ollama（ローカルLLM）で確認してください。

- Macbook air M2, 24Gバイトメモリー（次は、M6が欲しいですね。）
- PyCharm pro, Ollama API（OpenAI互換）, Python, Streamlit, docker compose(Redis, Qdrant)
...
- LLM（Ollama API（OpenAI互換））を使用した構造認識
...
チャンクはLLM(Ollama API（OpenAI互換）)で分割していますので、最重要は、prompts.pyのプロンプトです。
機能、性能はこのプロンプトで決まります。
...
flowchart TB
    ...
    subgraph EXTERNAL["外部サービス層"]
        GEMINI["Ollama API<br/>gemma4:e4b"]
    end
...
class CLI,MAIN,CORE,MODELS,PROMPTS,GEMINI default
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class MAIN,CORE,SR,PU,SU,PSP default
style STEP1_PY fill:#1a1a1a,stroke:#fff,color:#fff
...
| ライブラリ | バージョン | 用途 |
|-----------|-----------|------|
| `openai` | >= 1.100.2 | Ollama API（OpenAI互換）クライアント |

### 2.3 標準ライブラリ依存

| モジュール | 用途 |
|-----------|------|
| `os` | 環境変数（任意の `OLLAMA_BASE_URL`）の取得 |
...
**概要**: テキストを空行（`\n\n`）ベースで段落単位に分割する。LLM（Ollama API（OpenAI互換））を使用して構造を認識。
...
|------------|------|-----------|------|
| `text` | str | - | 入力テキスト |
| `api_key` | str | - | Ollama 互換クライアント用キー（ローカル実行のため `"ollama"` 固定で可） |
| `block_size` | int | 2000 | ブロックサイズ（文字数） |

| 項目 | 内容 |
|------|------|
| **Input** | `text: str`, `api_key: str`, `block_size: int = 2000` |
| **Process** | 1. `OpenAI`（Ollama互換）クライアントを初期化<br>2. **[v1.1追加]** `preprocess_text()`で前処理（長い行を句読点で分割）<br>3. テキストを`block_size`単位でブロック分割<br>4. 各ブロックに`PARAGRAPH_SEPARATION_PROMPT`を適用<br>5. Ollama API呼び出し（JSONモード `response_format={"type": "json_object"}`、スキーマはシステムプロンプトに埋め込み）<br>6. `StructuralResult.model_validate_json()`でパース<br>7. 各段落の`full_text`を抽出してリストに追加<br>8. **[v1.1追加]** 各段落を`postprocess_paragraph()`で後処理 |
...
def step1_hierarchical_split(text: str, api_key: str, block_size: int = 2000) -> list[str]:
    ...
    Args:
        text: 入力テキスト
        api_key: Ollama 互換クライアント用キー（ローカル実行のため "ollama" 固定で可）
        block_size: ブロックサイズ（文字数）
    Returns:
        段落のリスト
    """
    # ① Ollama API（OpenAI互換）クライアントを初期化
    ...
    for i, block in enumerate(blocks):
        ...
        # ⑥ Ollama API（OpenAI互換）呼び出し（同期）
        #    JSONモードを使用し、スキーマはシステムプロンプトに埋め込む
        response = client.chat.completions.create(
            model="gemma4:e4b",  # 既定（代替: llama3.2）
            messages=[
                {"role": "system", "content": f"{prompt}\n\n必ず StructuralResult のJSONスキーマに従うJSONのみを出力してください。"},
            ],
            response_format={"type": "json_object"},
        )

        # ⑦ レスポンスをパース
        result = StructuralResult.model_validate_json(response.choices[0].message.content)
...
# Ollama はローカル実行のため API キー不要（"ollama" 固定で可）
api_key = "ollama"
...
| 項目 | 内容 |
|------|------|
| **Input** | なし（API キー不要。任意で `OLLAMA_BASE_URL` を使用） |
| **Process** | 1. API キーは不要（ローカル実行、`"ollama"` 固定で可）<br>2. テスト用テキスト（5段落構成）を準備<br>3. `step1_hierarchical_split()`を呼び出し<br>4. 結果を表示・検証（期待値: 5段落）<br>5. Step2・Step3との連携情報を表示 |
...
| `model` | "gemma4:e4b" | 使用するOllamaモデル（代替: `llama3.2`） |
...
ローカル実行のため API コストは発生しない（トークン集計のみ）。

| モデル | 特性 |
|--------|----|
| **gemma4:e4b** | 【既定推奨】軽量・高速。段落分割に十分な精度 |
| llama3.2 | 【代替】汎用バランス型 |
| qwen2.5:7b | 【高精度】複雑な判断向け（メモリ余裕がある場合） |

### 5.4 環境変数

| 環境変数 | 必須 | 説明 |
|---------|:----:|------|
| `OLLAMA_BASE_URL` | 任意 | Ollama API（OpenAI互換）のベースURL（既定 http://localhost:11434/v1） |

> ℹ️ Ollama はローカル実行のため **API キーは不要** です（クライアントには `"ollama"` を渡せば可）。
...
# 任意でベースURLを設定（既定 http://localhost:11434/v1）
...
| バージョン | 変更内容 |
|-----------|---------|
| 1.2 | Ollama ネイティブ化（Gemini→Ollama 表記/コード/依存の全面置換）（2026-06-21） |
| 1.0 | 初版作成（Step1単体確認用プログラム） |
| 1.1 | 前処理・後処理関数追加（`preprocess_text()`, `postprocess_paragraph()`）、デフォルトモデルを更新、`chunk_text`を使用した日本語・英語対応の文分割 |
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class STEP1,GENAI,TYPES,MODELS,PROMPTS,OS,SR,PU,SU,PSP default
...
- 段落（パラグラフ）への分割は、regexでも可能ですが、
  LLM（ここではOllama（ローカルLLM））のAPIを使う練習ですね。
...
Step1では、LLM（Ollama API（OpenAI互換））を活用して、これらの問題を解決します。
...
| 2 | 各ブロックをLLM（Ollama API（OpenAI互換））に送信<br>・JSONモード（`response_format={"type": "json_object"}`）<br>・Pydanticスキーマ（`StructuralResult`）をシステムプロンプトに埋め込み、`model_validate_json()`でパース |
...
利用するモデルのデフォルトは軽量・高速の"gemma4:e4b"としていますが、
他のローカルモデル（`llama3.2` 等）も利用し、結果を比較してみましょう。（段落分けではあまり差はない。）

```python
response = client.chat.completions.create(
    model="gemma4:e4b",                          # モデル名（既定、代替: llama3.2）
    messages=[
        {"role": "system", "content": prompt},   # プロンプト＋スキーマ説明を埋め込み
    ],
...
| パラメータ | 値 | 説明 |
|------------|-----|------|
| `model` | `"gemma4:e4b"` | 既定の軽量モデル。ローカル実行で低レイテンシ |
| `response_format` | `{"type": "json_object"}` | JSON形式のレスポンスを要求（Ollama JSONモード） |
| スキーマ | `StructuralResult` | Pydanticスキーマをシステムプロンプトに埋め込み、`model_validate_json()`でパース |
...
| 方式 | LLM（Ollama API（OpenAI互換））による構造認識 |
