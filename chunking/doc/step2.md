...
Step2（意味的分割）はLLMのAPI(Ollama API（OpenAI互換）)を利用して意味的分割を実施します。
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class MAIN,CORE,SR,PU,SU,SCP default
style STEP2_PY fill:#1a1a1a,stroke:#fff,color:#fff
...
**概要**: 段落を意味的な類似度に基づいてチャンクに分割する。LLM（Ollama API（OpenAI互換））を使用して話題の転換点を検出。
...
| `api_key` | str | "ollama" | Ollama はローカル実行のため任意（既定値 `"ollama"` でよい） |

| 項目 | 内容 |
|------|------|
| **Input** | `paragraphs: list[str]`, `api_key: str`（ローカル実行のため任意） |
| **Process** | 1. `OpenAI`（Ollama互換）クライアントを初期化<br>2. 各段落に`SEMANTIC_CHUNKING_PROMPT`を適用<br>3. Ollama API呼び出し（`response_format={"type": "json_object"}`）<br>4. `StructuralResult.model_validate_json()`でパース<br>5. 各チャンクの`full_text`を抽出してリストに追加 |
...
def step2_semantic_chunking(paragraphs: list[str], api_key: str) -> list[str]:
    ...
    Args:
        ...
        api_key: Ollama はローカル実行のため任意（既定値 "ollama" でよい）
    ...
    for i, para in enumerate(paragraphs):
        ...
        # ③ プロンプト作成（スキーマをシステムプロンプトに埋め込み + 入力テキスト）
        prompt = f"{SEMANTIC_CHUNKING_PROMPT}\n\n【入力テキスト】\n{para}"

        # ④ Ollama API 呼び出し（JSON モード・同期）
        ...
        # ⑤ レスポンスをパース
        result = StructuralResult.model_validate_json(
...
# Ollama はローカル実行のため API キー不要（既定値 "ollama" でよい）
api_key = "ollama"
...
| 項目 | 内容 |
|------|------|
| **Input** | なし（ローカル実行のため API キー不要。任意で `OLLAMA_BASE_URL` を使用） |
| **Process** | 1. ローカル実行のため API キーは不要（任意で `OLLAMA_BASE_URL` を参照）<br>2. テスト用段落（5段落構成）を準備<br>3. `step2_semantic_chunking()`を呼び出し<br>4. 結果を表示・検証（期待値: 10チャンク）<br>5. Step3との連携情報を表示 |
...
ローカル実行のため API コストは発生しない（トークン集計のみ）。用途に応じて以下のモデルを選択する。

| モデル | 特性 |
|--------|------|
| **gemma4:e4b** | 【既定推奨】軽量で大量処理に最適 |
| llama3.2 | 【バランス】汎用的な意味的分割向け |
| qwen2.5:7b | 【高性能】複雑な判断・最終推論向け |
...
# 任意で接続先を変更する場合のみ OLLAMA_BASE_URL を設定
...
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class STEP2,GENAI,TYPES,MODELS,PROMPTS,OS,SR,PU,SU,SCP default
...
| 2 | 各段落をLLM（Ollama API（OpenAI互換））に送信<br>・JSONモード（`response_format={"type": "json_object"}`）<br>・スキーマをシステムプロンプトに埋め込み、`StructuralResult.model_validate_json()`でパース |
...
# スキーマはシステムプロンプトに埋め込み、出力を model_validate_json() でパース
result = StructuralResult.model_validate_json(
    response.choices[0].message.content
)
```
...
| パラメータ | 値 | 説明 |
|------------|-----|------|
| `model` | `"gemma4:e4b"` | 既定推奨のローカルモデル（代替: `llama3.2`） |
| `response_format` | `{"type": "json_object"}` | JSONモードのレスポンスを要求 |
| スキーマ指定 | システムプロンプト埋め込み | `StructuralResult.model_validate_json()` でパース |
...
| 出力 | チャンクリスト（数が増える可能性） |
| 目的 | 意味的な類似度に基づいた分割 |
| 方式 | LLM（Ollama API（OpenAI互換））による話題転換検出 |
